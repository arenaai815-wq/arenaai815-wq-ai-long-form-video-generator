      ...
      {logs.data ? (
        <div className="space-y-2">
          ...
          <pre className="max-h-[50vh] overflow-auto rounded-lg bg-black/40 p-3 font-mono text-[11px] leading-relaxed">
            {logs.data.logs.length ? logs.data.logs.map((l, i) => <div key={i} className={cn(l.level === "error" && "text-red-300", l.level === "warning" && "text-amber-300")}>{formatDate(l.ts)} [{l.level}] {l.message}</div>) : "No log entries."}
          </pre>
